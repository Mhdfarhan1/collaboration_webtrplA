    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 reveal">
        @forelse($projects as $project)
            <div
                ...
                <div class="px-3 pt-3 pb-2 flex flex-col h-full">
                    <!-- Top Meta: Semester Badge & Manpro Chip -->
                    <div class="flex items-center justify-between gap-2 mb-3">
                        <span class="px-2.5 py-1 rounded-lg bg-blue-50 text-blue-700 text-[10px] font-extrabold uppercase border border-blue-100">
                            Semester {{ $project->semester ?? 1 }}
                        </span>

                        @if($project->projectManager)
                            <div class="flex items-center gap-1.5 bg-slate-50 px-2 py-1 rounded-full border border-slate-200" title="Manpro: {{ $project->projectManager->lecturer_name }}">
                                <div class="w-5 h-5 rounded-full overflow-hidden border border-slate-300 shrink-0 bg-slate-200 flex items-center justify-center">
                                    ...
                                    @else
                                        <span class="text-[9px] font-bold text-slate-600">{{ substr($project->projectManager->lecturer_name, 0, 1) }}</span>
                                    @endif
                                </div>
                                <span class="text-[10px] font-bold text-slate-700 max-w-[100px] truncate">Manpro: {{ $project->projectManager->lecturer_name }}</span>
                            </div>
                        @endif
                    </div>

                    <h3
                        class="text-xl font-black text-slate-800 mb-2 leading-tight group-hover:text-brand-600 transition-colors line-clamp-1">
                        {{ $project->title }}
                    </h3>
                        ...
                        <div class="flex -space-x-2">
                            @foreach($project->projectMembers->take(3) as $pm)
                                @if($pm->member && $pm->member->member_image)
                                    <img class="w-7 h-7 rounded-full border-2 border-white shadow-xs object-cover"
                                        src="{{ asset($pm->member->member_image) }}" alt="{{ $pm->member->member_name }}"
                                        title="{{ $pm->member->member_name }} - {{ $pm->project_member_role }}">
                                @else
                                    <div class="w-7 h-7 rounded-full border-2 border-white shadow-xs bg-slate-200 flex items-center justify-center text-[10px] font-bold text-slate-500"
                            ...
                            @if($project->projectMembers->count() > 3)
                                <div
                                    class="w-7 h-7 rounded-full border-2 border-white shadow-xs bg-slate-100 flex items-center justify-center text-[9px] font-bold text-slate-600">
                    ...
                    <!-- Bottom Divider & Technology Badges at Bottom -->
                    <div class="pt-3 border-t border-slate-100 flex items-center justify-between gap-2">
                        <div class="flex flex-wrap gap-1.5">
                            @forelse($project->projectTechs->take(3) as $tech)
                                <span
                                    class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-slate-100/90 text-slate-700 text-[10px] font-extrabold uppercase border border-slate-200/80">
                        ...
                        <a href="{{ route('projects.detail', \App\Helpers\SecurityHelper::encode($project->project_id)) }}"
                            class="flex items-center gap-1.5 text-xs font-bold text-slate-800 hover:text-brand-600 transition-colors group/link shrink-0">
