...
@section('content')
    <div class="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto">
        ...
        <div class="mb-6 sm:mb-8 mt-6 sm:mt-10 flex items-center justify-between">
            <div>
                <h2 class="text-2xl sm:text-3xl font-extrabold text-slate-800 tracking-tight">Edit Tautan</h2>
                <p class="text-sm text-slate-500 mt-1">Ubah data tautan yang sudah ada.</p>
            </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden max-w-3xl">
            <form action="{{ route('admin.links.update', $link->link_id) }}" method="POST" class="p-6 sm:p-8">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <select id="link_type" name="link_type" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 text-slate-800 rounded-xl focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 transition-all sm:text-sm">
                            ...
                            <option value="notion" {{ old('link_type', $link->link_type) == 'notion' ? 'selected' : '' }}>Notion Kelas</option>
                            <option value="instagram" {{ old('link_type', $link->link_type) == 'instagram' ? 'selected' : '' }}>Instagram</option>
                            <option value="schedule" {{ old('link_type', $link->link_type) == 'schedule' ? 'selected' : '' }}>Jadwal Matkul</option>
                        </select>
                        @error('link_type')
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="url" id="link_url" name="link_url" value="{{ old('link_url', $link->link_url) }}" required placeholder="https://..." class="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 text-slate-800 rounded-xl focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 transition-all sm:text-sm">
                        </div>
                        @error('link_url')
                            <p class="mt-1.5 text-sm text-red-500 flex items-center gap-1.5"><i data-lucide="alert-circle" class="w-4 h-4"></i> {{ $message }}</p>
                        @enderror
                ...
                <div class="mt-8 pt-6 border-t border-slate-100 flex justify-end">
                    <button type="submit" class="inline-flex items-center gap-2 px-6 py-2.5 bg-brand-600 text-white text-sm font-semibold rounded-xl hover:bg-brand-700 focus:ring-4 focus:ring-brand-100 transition-all shadow-sm">
                        <i data-lucide="save" class="w-4 h-4"></i> Perbarui Tautan
                    </button>
                </div>
            </form>
        </div>
