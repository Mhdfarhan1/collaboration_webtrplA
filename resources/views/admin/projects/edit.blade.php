    ...
    <div class="p-4 sm:p-6 lg:p-8 max-w-4xl mx-auto">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
            <form action="{{ route('admin.projects.update', $project->project_id) }}" method="POST"
                ...
                <!-- Tipe Project & Semester (2 Column Grid) -->
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div>
                        <label for="project_type" class="block text-sm font-semibold text-slate-700 mb-2">Tipe Project <span
                                class="text-red-500">*</span></label>
                        <select id="project_type" name="project_type" required
                            class="w-full px-4 py-2.5 bg-slate-50 border border-slate-300 rounded-lg focus:bg-white focus:ring-2 focus:ring-brand-500 focus:border-brand-500 outline-none transition-all cursor-pointer">
                            @php $currentType = old('project_type', $project->project_type ?? 'Web Application'); @endphp
                            <option value="Web Application" {{ $currentType == 'Web Application' ? 'selected' : '' }}>Web Application</option>
                            <option value="Mobile Application" {{ $currentType == 'Mobile Application' ? 'selected' : '' }}>Mobile Application</option>
                            <option value="IoT & Hardware" {{ $currentType == 'IoT & Hardware' ? 'selected' : '' }}>IoT & Hardware</option>
                            <option value="Desktop Application" {{ $currentType == 'Desktop Application' ? 'selected' : '' }}>Desktop Application</option>
                            <option value="AI & Data Science" {{ $currentType == 'AI & Data Science' ? 'selected' : '' }}>AI & Data Science</option>
                            <option value="Lainnya" {{ $currentType == 'Lainnya' ? 'selected' : '' }}>Lainnya</option>
                        </select>
                        @error('project_type')
                            <p class="text-red-500 text-sm mt-1.5">{{ $message }}</p>
                        @enderror
                ...
                <!-- Manpro & Link Demo (2 Column Grid) -->
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div>
