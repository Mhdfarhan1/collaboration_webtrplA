...
@section('content')
    <div class="p-4 sm:p-6 lg:p-8 max-w-4xl mx-auto">
        ...
        <form action="{{ route('admin.lecturers.update', $lecturer->lecturer_id) }}" method="POST" enctype="multipart/form-data" novalidate class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div id="additional-details" class="space-y-6">
                        ...
                        <!-- Education Section -->
                        @php
                            $d3_campus = old('edu_d3_campus');
                            $d3_major = old('edu_d3_major');
                            $s1_campus = old('edu_s1_campus');
                            $s1_major = old('edu_s1_major');
                            $s2_campus = old('edu_s2_campus');
                            $s2_major = old('edu_s2_major');
                            $s3_campus = old('edu_s3_campus');
                            $s3_major = old('edu_s3_major');

                            if (is_null($d3_campus) && is_null($s1_campus) && is_null($s2_campus) && is_null($s3_campus) && !empty($lecturer->education_history)) {
                                ...
                                foreach ($lines as $line) {
                                    ...
                                    if (str_contains($line, ':')) {
                                        [$title, $major] = array_map('trim', explode(':', $line, 2));
                                    ...
                                    if (preg_match('/^(Diploma\s*(?:\(DIII\)|\(D3\)|\(DIV\)|\(D4\))?|D3|DIII|D4|DIV)\s*(.*)$/i', $title, $m)) {
                                        $d3_campus = $m[2] ? trim($m[2]) : $title;
                                        $d3_major = $major;
                                    } elseif (preg_match('/^(Sarjana\s*(?:\(S1\))?|S1)\s*(.*)$/i', $title, $m)) {
                                        $s1_campus = $m[2] ? trim($m[2]) : $title;
                                        $s1_major = $major;
                                    } elseif (preg_match('/^(Magister\s*(?:\(S2\))?|S2)\s*(.*)$/i', $title, $m)) {
                                        $s2_campus = $m[2] ? trim($m[2]) : $title;
                                        $s2_major = $major;
                                    } elseif (preg_match('/^(Doktor\s*(?:\(S3\))?|S3)\s*(.*)$/i', $title, $m)) {
                                        $s3_campus = $m[2] ? trim($m[2]) : $title;
                                        $s3_major = $major;
                                    }
                                }
                            }
                            ...
                            <div class="space-y-5">
                                ...
                                <div class="p-4 bg-slate-50/80 rounded-2xl border border-slate-200/80 space-y-3">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                        <div>
                                            ...
                                            <input type="text" name="edu_d3_campus" value="{{ $d3_campus }}"
                                        ...
                                        <div>
                                            ...
                                            <input type="text" name="edu_d3_major" value="{{ $d3_major }}"
                                                class="w-full px-3.5 py-2 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-brand-500 outline-none text-xs font-medium"
                                ...
                                <div class="p-4 bg-slate-50/80 rounded-2xl border border-slate-200/80 space-y-3">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                        <div>
                                            <label class="block text-[10px] font-bold text-slate-500 mb-1">Nama Kampus / Institusi</label>
                                            <input type="text" name="edu_s1_campus" value="{{ $s1_campus }}"
                                                class="w-full px-3.5 py-2 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-brand-500 outline-none text-xs font-medium"
                                                placeholder="Contoh: Institut Teknologi Bandung">
                                        </div>
                                        <div>
                                            <label class="block text-[10px] font-bold text-slate-500 mb-1">Jurusan / Program Studi</label>
                                            <input type="text" name="edu_s1_major" value="{{ $s1_major }}"
                                                class="w-full px-3.5 py-2 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-brand-500 outline-none text-xs font-medium"
                                ...
                                <div class="p-4 bg-slate-50/80 rounded-2xl border border-slate-200/80 space-y-3">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                        <div>
                                            <label class="block text-[10px] font-bold text-slate-500 mb-1">Nama Kampus / Institusi</label>
                                            <input type="text" name="edu_s2_campus" value="{{ $s2_campus }}"
                                                class="w-full px-3.5 py-2 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-brand-500 outline-none text-xs font-medium"
                                                placeholder="Contoh: Universitas Gadjah Mada">
                                        </div>
                                        <div>
                                            <label class="block text-[10px] font-bold text-slate-500 mb-1">Jurusan / Program Studi</label>
                                            <input type="text" name="edu_s2_major" value="{{ $s2_major }}"
                                                class="w-full px-3.5 py-2 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-brand-500 outline-none text-xs font-medium"
